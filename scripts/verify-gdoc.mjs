...
// исходника и ищет его в тексте, вычитанном обратно из документа.
//
// Использование:
//   node scripts/verify-gdoc.mjs <каталог --gdoc> <NN> <файл с текстом документа>
//
// Пустой список расхождений = документ совпал с выгрузкой дословно.
import fs from 'fs';
import path from 'path';

const [dir, nn, readbackPath] = process.argv.slice(2);
if (!dir || !nn || !readbackPath) {
  console.error('usage: node scripts/verify-gdoc.mjs <gdoc-dir> <NN> <readback.txt>');
  process.exit(2);
}
...
const md = fs.readFileSync(path.join(dir, `${nn}.md`), 'utf8');
...
console.log(`страница ${nn}: проверено фрагментов ${checked}, не найдено ${missing.length}`);
